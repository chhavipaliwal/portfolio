...
const cmdk = tv({
  slots: {
    ...
    leftIcon: ['text-default-500', 'group-data-[active=true]:text-primary-foreground'],
...
const CommandMenu: FC<{}> = () => {
  ...
  const [selectedCategory, setSelectedCategory] = useState<CategoryEnum>(CategoryEnum.PROJECTS);
      ...
      if (isRecent) {
        ...
        return (
          <Popover key={item.slug} open={isActive && !isMobile}>
            ...
            <PopoverContent
              className={cn(
                'h-[160px] w-[305px] overflow-hidden rounded-[14px] border border-default-400/50'
              )}
              side={'right'}
              sideOffset={24}
            >
              <Card
                className={cn(
                  'bg-grid-slate-900/70 mask-[linear-gradient(0deg,#fff,rgba(255,255,255,0.6))] relative h-full w-full overflow-hidden'
                )}
              >
                <div
                  aria-hidden="true"
                ...
                >
                  <div
                    className="aspect-1155/678 bg-linear-to-tr w-full from-[#f13a76] to-[#2e83f5] opacity-40"
                    style={{
                      clipPath:
                ...
                <CardBody className="aspect-2/1 after:bg-linear-to-t relative overflow-visible bg-black/40 p-0 before:absolute after:absolute after:inset-x-0 after:bottom-0 after:z-20 after:h-16 after:rounded-medium after:from-black after:to-transparent after:content-['']">
                  <Image
                    alt={`${item?.component?.name}`}
                    className={cn(
